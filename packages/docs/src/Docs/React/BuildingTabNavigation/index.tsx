...
export const ReactDocsOuterEventsHandler = (props: RouteComponentProps) => {
  ...
  return (
    <DocsContentPage>
      ...
      <p>Easily build tabs with only two components. Easily control the size, content and more. </p>

      <ol>
        <li><Link to="/docs/react/tabs">Tabs (container)</Link></li>
        <li><Link to="/docs/react/tab-item">TabItem</Link></li>
      </ol>

      <h2>Basic Usage</h2>
      <p>By default, wrapping Tabs component renders <code>nav</code> element and TabItem renders <code>a</code> element. NavItem has tabindex="0" property by default. Add class "active" to render active state. </p>


      <CodeExample code={`<Tabs>
  ...
  <TabItem>History</TabItem>
      ...
      <CodeExample code={`const WidgetWithTabs = (props) => {
  // router props
  const { match } = props
  return (
    <Widget>
