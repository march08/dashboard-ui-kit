...
export const ReactDocsOuterEventsHandler = () => {
          ...
          content: (
            <OuterEventsHandler onOuterEvent={handleOuterEvent}>
              <div style={{ padding: 50, background: 'orange', color: 'white' }}>
                Try to click outside/inside this content or press esc<br />
                Outer Events: {clicks}
              </div>
            </OuterEventsHandler>
          ),
  ...
  <div style={{ padding: 50, background: 'orange', color: 'white' }}>
    ...
    Outer Events: {clicks}
      ...
      <p>This doesn't look that useful so far, let's see a real example of OuterEventsHandler in action with <Link to="/docs/react/dropdown">Dropdown</Link>, which is wrapped by OuterEventsHandler.</p>
      ...
      <p>As mentioned above, OuterEventsHandler can trigger a passed function on different circumstances, even on window resize or scroll. We can pass those bool props directly to the Dropdown. Dropdown doesn't use those boolean properties directly, it passes them down to the OuterEventsHandler instead.</p>
