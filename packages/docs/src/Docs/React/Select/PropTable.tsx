...
const optionProps = [
  ...
  properties.children({
    ...
    desc: 'Display value to be rendered on the select button or select option. Value will be used as fallback if not provided.'
  }),
  {
    prop: 'props',
    propType: 'props of option component (Button)',
    desc: 'You can pass props to the option component as well, such as "disabled"'
  }
]
...
const itemProps = [
  ...
  properties.bool({
    prop: 'activeOption',
    propType: <><Link to="#select-option">SelectOption</Link> | <Link to="#select-option">SelectOption</Link>[]</>,
    desc: 'Active option(s) to be displayed. Single or Array.'
  }),
  properties.bool({
    prop: 'options',
    propType: <><Link to="#select-option">SelectOption</Link>[]</>,
  ...
  }),
  {
    prop: 'name',
    propType: 'string',
    desc: 'If you need to know on what select the onOptionClick has been triggered, pass a name value.'
  },
  {
    prop: 'onOptionClick',
    propType: <span style={{ whiteSpace: 'nowrap' }}>(option: SelectOption, name?: string)<br />&nbsp;&nbsp;=> void</span>,
